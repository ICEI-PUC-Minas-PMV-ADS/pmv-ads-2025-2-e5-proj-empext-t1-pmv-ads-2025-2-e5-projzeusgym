...
// =================================================================
// DELETAR FICHA - Apenas Professor/Admin
// =================================================================
exports.deleteTrainingSheet = async (req, res) => {
  try {
    ...
    // Verificar se o usuário é professor ou admin
    if (role === 'aluno') {
      return res.status(403).json({ error: 'Acesso negado. Apenas professores podem deletar fichas.' });
    }
    ...
    if (!sheet) {
      return res.status(404).json({ error: 'Ficha não encontrada ou você não tem permissão para deletá-la.' });
    }

    // 1. Primeiro deletar todas as associações de exercícios
    await TrainingSheetExercises.destroy({
      where: { sheetId: sheetId }
    });

    // 2. Depois deletar a ficha
    const deletedSheet = await TrainingSheet.destroy({
      where: whereCondition
    });

    if (deletedSheet === 0) {
      return res.status(404).json({ error: 'Ficha não foi deletada.' });
    }

    return res.status(200).json({
      message: `Ficha '${sheet.nome}' deletada com sucesso!`
    });
